...
const body=document.body,menu=document.querySelector(".header__menu");function toggleElement(e,t){t.classList.contains("active")?(t.classList.remove("active"),e.classList.remove("lock")):(t.classList.add("active"),e.classList.add("lock"))}function burgerHandler(){requestAnimationFrame(()=>toggleElement(body,menu))}const popup=document.querySelector(".popup"),requestButtons=document.querySelectorAll(".request__btn"),closePopup=document.querySelector(".popup__close");function popupHandler(){requestAnimationFrame(()=>toggleElement(body,popup))}function hidePopup(){popup.classList.remove("active"),body.classList.remove("lock")}function toggleHeader(e){let n=window.scrollY||document.documentElement.scrollTop,r,o=0,a=0;const s=document.querySelector(".header");window.addEventListener("scroll",function(){if(r=window.scrollY||document.documentElement.scrollTop,r>n?o=2:r<n&&(o=1),o!==a){var e=o,t=r;if(e===2&&t>100){s.classList.add("hide");a=e}else if(e===1){s.classList.remove("hide");a=e}}n=r})}function addFixedClassToHeader(){const e=document.querySelector(".header");let t=e.offsetHeight+300;window.addEventListener("scroll",function(){this.scrollY>t?e.classList.add("fixed"):e.classList.remove("fixed")})}const form=document.querySelector(".popup__form"),inputs=Array.from(document.querySelectorAll(".popup__input"));function addErrorMsg(e,t){e.parentElement.nextElementSibling.classList.add("error");const n=e.parentElement.previousElementSibling;n.textContent=t,n.classList.add("active")}function showError(e){"name"===e.name?e.validity.valueMissing?addErrorMsg(e,"Fill in the 'name' fild."):e.validity.tooShort&&addErrorMsg(e,"Name should be at least 3 characters."):"phone"===e.name&&e.value.includes("_")&&addErrorMsg(e,"Type correct phone number.")}function createCard(e){var t=e.id%10==0?1:e.id%10;const n=document.createElement("DIV");return n.classList.add("cards__item","card"),n.innerHTML=`
  <div class="card__picture"><img src="./images/cards/card-${t}.png" alt=""/></div>
  ...
  `,n}let postsData=[],counter=1;const cardsWrapper=document.querySelector(".cards__wrapper");function renderCards(e,t){if(counter<21){const o=document.createDocumentFragment();for(var n=counter+5;counter<n;){var r=createCard(e[counter]);o.append(r),counter+=1}if(t.append(o),21<=counter){const a=document.querySelector(".cards__more");a.classList.add("disabled")}}}function loadMoreHandler(){0===postsData.length?fetch("https://jsonplaceholder.typicode.com/posts").then(e=>e.json()).then(e=>{postsData=[postsData].concat(e),renderCards(postsData,cardsWrapper)}):renderCards(postsData,cardsWrapper)}document.addEventListener("DOMContentLoaded",function(e){const t=document.querySelector(".header__burger"),n=(window.innerWidth<=1599&&t.addEventListener("click",burgerHandler),window.matchMedia("(max-width: 1599px)"));function r(e){const t=e.parentElement.previousElementSibling;t.textContent="",t.className="popup__error",e.parentElement.nextElementSibling.classList.remove("error")}n.addEventListener("change",function(e){e.matches?t.addEventListener("click",burgerHandler):(t.removeEventListener("click",burgerHandler),menu.classList.remove("active"),body.classList.remove("lock"))}),requestButtons.forEach(e=>e.addEventListener("click",popupHandler)),closePopup.addEventListener("click",hidePopup),toggleHeader(),addFixedClassToHeader(),inputs.forEach(e=>e.addEventListener("input",()=>{("name"===e.name&&e.validity.valid||"phone"===e.name&&e.value.includes("_")?r:showError)(e)})),form.addEventListener("submit",function(t){inputs.forEach(e=>{"name"===e.name?e.validity.valid||(t.preventDefault(),showError(e)):"phone"===e.name&&e.value.includes("_")&&(t.preventDefault(),showError(e))})});var o=document.getElementById("phoneInput");new IMask(o,{mask:"+{7}(000)000-00-00",lazy:!1});const a=document.querySelector(".cards__more");a.addEventListener("click",()=>{loadMoreHandler()}),setTimeout(function(){body.classList.remove("animated")},100)});