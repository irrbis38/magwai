...
const body=document.body,menu=document.querySelector(".header__menu");function toggleElement(e,t){t.classList.contains("active")?(t.classList.remove("active"),e.classList.remove("lock")):(t.classList.add("active"),e.classList.add("lock"))}function burgerHandler(){requestAnimationFrame(()=>toggleElement(body,menu))}const popup=document.querySelector(".popup"),requestButtons=document.querySelectorAll(".request__btn"),closePopup=document.querySelector(".popup__close");function popupHandler(){requestAnimationFrame(()=>toggleElement(body,popup))}function hidePopup(){popup.classList.remove("active"),body.classList.remove("lock")}function toggleHeader(e){let r=window.scrollY||document.documentElement.scrollTop,n,a=0,o=0;const s=document.querySelector(".header");window.addEventListener("scroll",function(){if(n=window.scrollY||document.documentElement.scrollTop,n>r?a=2:n<r&&(a=1),a!==o){var e=a,t=n;if(e===2&&t>100){s.classList.add("hide");o=e}else if(e===1){s.classList.remove("hide");o=e}}r=n})}function addFixedClassToHeader(){const e=document.querySelector(".header");let t=e.offsetHeight+300;window.addEventListener("scroll",function(){this.scrollY>t?e.classList.add("fixed"):e.classList.remove("fixed")})}const form=document.querySelector(".popup__form"),inputs=Array.from(document.querySelectorAll(".popup__input"));function addErrorMsg(e,t){e.parentElement.nextElementSibling.classList.add("error");const r=e.parentElement.previousElementSibling;r.textContent=t,r.classList.add("active")}function showError(e){"name"===e.name?e.validity.valueMissing?addErrorMsg(e,"Fill in the 'name' fild."):e.validity.tooShort&&addErrorMsg(e,"Name should be at least 3 characters."):"phone"===e.name&&e.value.includes("_")&&addErrorMsg(e,"Type correct phone number.")}function createCard(e){var t=e.id%10==0?1:e.id%10;const r=document.createElement("DIV");return r.classList.add("cards__item","card"),r.innerHTML=`
  <div class="card__picture">
  <picture class="picture object-fit-cover">
    <source class="object-fit-cover" srcset="images//cards/card-${t}.webp, images//cards/card-${t}@2x.webp 2x" type="image/webp"/><img class="picture__img object-fit-cover" src="images//cards/card-${t}.png" srcset="images//cards/card-${t}.png, images//cards/card-7@2x.png 2x" alt=""/>
  </picture>  
  </div>
  ...
  `,r}let postsData=[],counter=1;const cardsWrapper=document.querySelector(".cards__wrapper");function renderCards(e,t){if(counter<21){const a=document.createDocumentFragment();for(var r=counter+5;counter<r;){var n=createCard(e[counter]);a.append(n),counter+=1}if(t.append(a),21<=counter){const o=document.querySelector(".cards__more");o.classList.add("disabled")}}}function loadMoreHandler(){0===postsData.length?fetch("https://jsonplaceholder.typicode.com/posts").then(e=>e.json()).then(e=>{postsData=[postsData].concat(e),renderCards(postsData,cardsWrapper)}):renderCards(postsData,cardsWrapper)}document.addEventListener("DOMContentLoaded",function(e){const t=document.querySelector(".header__burger"),r=(window.innerWidth<=1599&&t.addEventListener("click",burgerHandler),window.matchMedia("(max-width: 1599px)"));function n(e){const t=e.parentElement.previousElementSibling;t.textContent="",t.className="popup__error",e.parentElement.nextElementSibling.classList.remove("error")}r.addEventListener("change",function(e){e.matches?t.addEventListener("click",burgerHandler):(t.removeEventListener("click",burgerHandler),menu.classList.remove("active"),body.classList.remove("lock"))}),requestButtons.forEach(e=>e.addEventListener("click",popupHandler)),closePopup.addEventListener("click",hidePopup),toggleHeader(),addFixedClassToHeader(),inputs.forEach(e=>e.addEventListener("input",()=>{("name"===e.name&&e.validity.valid||"phone"===e.name&&e.value.includes("_")?n:showError)(e)})),form.addEventListener("submit",function(t){inputs.forEach(e=>{"name"===e.name?e.validity.valid||(t.preventDefault(),showError(e)):"phone"===e.name&&e.value.includes("_")&&(t.preventDefault(),showError(e))})});var a=document.getElementById("phoneInput");new IMask(a,{mask:"+{7}(000)000-00-00",lazy:!1});const o=document.querySelector(".cards__more");o.addEventListener("click",()=>{loadMoreHandler()}),setTimeout(function(){body.classList.remove("animated")},100)});